...
import co.kr.dev.common.DBUtility;
...
public class StudentDAO {
    ...
    public StudentVO getStudentById(String id) throws SQLException {
        ...
        try {
            conn = DBUtility.dbCon(); // DB 연결
            pstmt = conn.prepareStatement(query);
            pstmt.setString(1, id); // ID 파라미터 바인딩
            rs = pstmt.executeQuery();

            if (rs.next()) {
                // 조회된 데이터를 StudentVO 객체에 담기
                student = new StudentVO();
                student.setId(rs.getString("ID"));
                student.setPass(rs.getString("PW"));
                student.setName(rs.getString("NAME"));
                String phone = rs.getString("PHONE");
                if (phone != null && phone.length() >= 3) {
                    student.setPhone1(phone.substring(0, 3));
                    student.setPhone2(phone.substring(3, 7));
                    student.setPhone3(phone.substring(7));
                }
                student.setEmail(rs.getString("EMAIL"));
                student.setZipcode(rs.getString("ZIP"));
                student.setAddress1(rs.getString("ADDR1"));
                student.setAddress2(rs.getString("ADDR2"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
            throw new SQLException("학생 정보를 불러오는 중 오류 발생");
        } finally {
            // 리소스 해제
            DBUtility.dbClose(conn, pstmt, rs);
        }
        return student;
    }
}
